...
router.put('/like-post', validate, async (req, res) => {
  try {
    ...
    let like = await CreatePostModel.findByIdAndUpdate({ _id: req.body.id }, {
      $push: { likes: user._id }
    }, {
      new: true
    })
...
router.put('/unLike-post',validate, async (req, res) => {
  try {
    ...
    let unLike = await CreatePostModel.findByIdAndUpdate({ _id: req.body.id }, {
      $pull: { likes: user._id }
    }, {
      new: true
    })
...
router.put('/comment-post',validate, async (req, res) => {
  try {
    ...
    let commentDetails = await CreatePostModel.findByIdAndUpdate({ _id: req.body.id }, {
