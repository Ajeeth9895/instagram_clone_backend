...
const { hashPassword, hashCompare, createToken, decodeToken, validate, roleAdmin,  decodePasswordToken } = require('../config/auth');
  ...
  try {
    ...
    if (user) {
      if (await hashCompare(req.body.password, user.password)) {
        ...
        let validate = await UserModel.findByIdAndUpdate({ _id: user._id }, { validate: true })
...
router.put("/follow",validate, async (req, res) => {
  try {
    ...
    let followDetails = await UserModel.findByIdAndUpdate({ _id: req.body.userId }, {
    ...
    let followersDetails = await UserModel.findByIdAndUpdate({ _id: user._id }, {
...
router.put("/uploadProfilePic",validate, async (req, res) => {
  try {
    ...
    let saveProfile = await UserModel.findByIdAndUpdate({ _id: user._id }, {
    ...
    let save = await CreatePostModel.findOneAndUpdate({email:req.body.email }, {
